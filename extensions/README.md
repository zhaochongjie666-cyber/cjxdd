...
| `read-dir/` | `read_dir` tool: recursively read many files/directories in one token-efficient tool call, with safety limits; `.xdd/design/` must be read in separate anchor batches (`spec`, `architecture`, `wire`, `resilience`, etc.) |

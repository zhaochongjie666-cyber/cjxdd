...
start
阶段一（需求研究 / 规格收敛）
（此阶段只允许读取 Design 文件。不允许读取代码。只关注我们要做成什么样子，有什么 Feature。核心业务流程是什么？）
输入：用户描述
1. 调研需求，
2. 编写 user 旅途,
2.5 用户角色模拟（agent 自主发散 7 类角色，每角色 10 维度深度档案，超深超重，周到布全）
3. 自我攻击，用户旅途实际是否符合？
4. 编写详细的 features,  
5. 自我攻击,  Feature 有无遗漏？有无矛盾？是否偏离了用户旅途？
6. 编写详细的可靠性，Feature 的可靠性、断点恢复、解耦性。
7. 自我攻击  是否有纰漏？

输出：用户旅途， 用户角色模拟，Feature 文档。可靠性，测试方法论，

阶段二（架构设计 + 系统韧性）
（此阶段只允许读取 Design 文件。不允许读取代码。只关注系统怎么设计，模块怎么划分，数据怎么流动。）
1. 设计解耦的，模块化的，可单元测试的。
2. 系统级韧性：失败模式 / 兜底 / 恢复 / 韧性测试计划（依赖 architecture 的失败模型）
3. 自我攻击  架构一定要解耦。模块化，单点可测试。 模块化编程，组件抽象为基础能力

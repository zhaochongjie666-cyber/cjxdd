...
```
□ 没有"操作应该成功""系统应正确处理"等空泛成功/正确？
□ 没有不明确主体（"用户拥有相关权限"-> 具体角色名）？
□ 没有不明确数据（"超过限制"-> 具体数字）？
□ 没有实现细节冒充业务规则（assignee_id / 数据库字段）？
□ 没有假装精确（未经确认的时间/数量/错误码 -> 标@待确认）？
□ 没有营销词（高效/智能/赋能/闭环/生态）？
```
...
□ 开发能否看出需要实现哪些规则？
□ 测试能否根据场景准备明确数据？
□ 产品能否根据 Then 判断验收通过或失败？
□ 失败后哪些数据不能变化，是否写清楚？
□ 删了所有步骤实现代码后，这份文件是否仍然是一份完整业务规范？
```

### 常规自检

```
□ 先提取了业务事实（角色/对象/状态/前置/成功结果/失败结果/副作用）再写 Gherkin？
□ 建立了场景覆盖矩阵，八类逐类判断了适用/不适用？
□ 规则优先（先写 Rule 再写 Scenario），不是页面/操作脚本？
□ Scenario 用具体角色/对象ID/状态/数字，不是"用户""相关权限""超过限制"？
□ 每个 Feature 至少一个异常 Scenario（拒绝/失败/不存在/无权限/冲突）？
□ 失败场景写了"原状态不变""不产生副作用"？
□ 并发场景写了"只能一次成功""失败方收到提示"？
□ 外部依赖失败写了主业务是否成功？
□ 每条 RXX 至少 1 个 Feature 覆盖？
□ 每条 RXX 关联角色（PX），角色名来自 personas/_index.md？
□ When/Then 没有实现细节词（调度器/线程池/锁/CAS/重试/连接池/后台循环）？
□ 状态名/角色名/错误码跟 design + architecture + personas 一致，未知标"待确认"？

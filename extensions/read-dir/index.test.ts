...
describe("read_dir", () => {
	...
	it("目录文本超过 200000 字符时拒绝读取并要求缩小目录", () => {
		const cwd = mkdtempSync(join(tmpdir(), "read-dir-"));
		try {
			mkdirSync(join(cwd, "large", "nested"), { recursive: true });
			writeFileSync(join(cwd, "large", "a.md"), "a".repeat(120_000));
			writeFileSync(join(cwd, "large", "nested", "b.md"), "b".repeat(80_001));
			expect(() => readDir(cwd, { paths: ["large"] })).toThrow(
				/目录 large\/ 超过 200000 字符.*请读取更小单位目录/,
			);
		} finally { rmSync(cwd, { recursive: true, force: true }); }
	});

	it("目录不超过 200000 字符时仍可读取", () => {
		const cwd = mkdtempSync(join(tmpdir(), "read-dir-"));
		try {
			mkdirSync(join(cwd, "within-limit"), { recursive: true });
			writeFileSync(join(cwd, "within-limit", "a.md"), "a".repeat(100_000));
			writeFileSync(join(cwd, "within-limit", "b.md"), "b".repeat(100_000));
			const result = readDir(cwd, { paths: ["within-limit"], maxFileChars: 100_000, maxTotalChars: 210_000 });
			expect(result.details.discovered).toBe(2);
			expect(result.details.truncated).toEqual([]);
		} finally { rmSync(cwd, { recursive: true, force: true }); }
	});

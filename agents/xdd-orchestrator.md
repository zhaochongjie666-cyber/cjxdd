...
| 设计·规格 | `phase-design` | xdd-spec + xdd-architecture + xdd-wire | spec/{slug}/ RXX+feature + architecture/{slug}/ + wire/{page}/ | 三 skill 自检 + mermaid 渲染 |
| 设计·韧性 | `phase-resilience` | xdd-resilience | architecture/{slug}/resilience/ 5 文档 | resilience 自检 |
| 桥接·计划 | `phase-plan` | xdd-plan | plan/{slug}/plan.md（task 回指 RXX）| plan 自检（RXX 覆盖 + 禁占位符）|

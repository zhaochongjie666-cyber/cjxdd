...
const STAGE_ANGLES: Record<string, AttackAngle[]> = {
	...
	plan: [
		...
		{
			name: "计划语义攻击",
			description: "抓范围错误、过度设计、错误顺序和假精确",
			checks: [
				"任务是否能追溯到已批准的 spec/architecture/wire/resilience，而不是自行扩大范围？",
				"是否存在为当前目标无必要性的基础设施、抽象层或迁移（过度设计）？",
				"契约冻结、生产实现、集成和验证的先后关系是否现实？",
				"缺少 effort/capacity/calendar 时是否编造了具体日期或关键路径时长？",
			],
		},
		{
			name: "计划正向兜底攻击",
			description: "同时攻击任务的完成证据、失败处理和可恢复性",
			checks: [
				"每个任务是否有可观察完成证据，而不是把代码已写当作完成？",
				"happy path 是否有实施和验证动作？",
				"计划整体是否为适用的拒绝、依赖不可用或部分失败配对了拦截、降级、恢复或回炉任务？",
				"Gate 失败是否能指向具体 task/上游阶段和修复动作，而不是只写未通过？",
			],
		},
		{
			name: "计划变更攻击",
			description: "抓冲突并行、旧假设继续执行和静默改写",
			checks: [
				"标为并行的任务是否会修改相同文件、共享契约或争用同一不可并发能力？",
				"契约、依赖、文件范围或验收标准变化时，计划是否要求显式重规划？",
				"已完成且仍有效的证据是否保留，受影响任务是否明确重做或标记阻塞？",
			],
		},
	],
	execute: [

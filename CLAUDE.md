...
定位清楚 = 想清楚要干啥。这张图是**你和用户的共享地图**：你说"我在改韧性锚的兜底设计"，用户看图就知道那是哪个节点、产出落在 `design/architecture/{slug}/resilience/`、上游是 architecture。认知偏差在定位这一步被截住，不在代码里爆发。

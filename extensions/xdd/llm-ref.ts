...
export async function getAIGateLLM(): Promise<{ model: any; apiKey?: string; headers?: Record<string, string> } | null> {
	if (!modelRef || !modelRegistryRef) return null;
	try {
		const auth = await modelRegistryRef.getApiKeyAndHeaders(modelRef);
		if (!auth.ok) return null;
		return { model: modelRef, apiKey: auth.apiKey, headers: auth.headers };
	} catch {
		return null;
	}
}

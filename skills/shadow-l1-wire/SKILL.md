...
**生命周期角色**:`design_baseline` 设计基线(`wire.svg` + `data-*` 标注是 L1.5 / L5 UI 契约源) + `process_output` 过程产物(`wire-skeleton.svg` / `wire-content.svg`,Pass 3 完成后删除)。详见 `framework/shadow-schema.json:lifecycle_artifacts` → `wire-svg` / `wire-skeleton`。

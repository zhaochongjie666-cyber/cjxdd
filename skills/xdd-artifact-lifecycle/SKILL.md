---
name: xdd-artifact-lifecycle
alias: xdd·Artifact-Lifecycle
methodology: |
  xdd 工件生命周期分类 — 5 类角色(设计基线 / 过程产物 / 证据存档 / 控制标记 / 模板与实例)
  单一源真理: .xdd/xdd-schema.json:lifecycle_artifacts[]
  配套门禁: scripts/gate-check-lifecycle.sh (5 角色 × canonical_path 一致性)
description: |
  xdd 工件生命周期元 skill — 替代/补充旧的"跨迭代 vs 迭代作用域"位置二分法,
  回答"工件用多久 + 谁消费 + 改后会怎样"。
  5 类: design_baseline / process_output / evidence_archive / control_marker / template_instance.
  Phase 1: 在 .xdd/xdd-schema.json 登记所有工件在 5 角色下。
  Phase 2 (本 skill): 升级为可被 xdd-walker 装卸的元 skill,提供 5 条硬门禁规则。
...
# xdd·Artifact-Lifecycle — 工件生命周期元 skill

## 角色

把 xdd `.xdd/` 下 30+ 份工件按**生命周期角色**分成 5 类,让 xdd-walker、Reviewer、xdd-trace-init 有一个统一语言回答"这份产物用多久 + 改后会怎样"。

**核心价值**:
- 替代"跨迭代 vs 迭代作用域"位置二分法(只描述在哪,不说用多久)
- 让 5 类角色 = 5 种命运(累积 / 弃用 / 冻结 / 标记 / 模板)
- 让 stop-gate 漂移扫描、Reviewer chain 审计、xdd-trace-init 反向追溯用同一张表

**xdd-walker 信条**:
1. **5 类足矣** — 不要再发明第 6 类,不够就拆"模糊地带"到现有 5 类
2. **角色判定看命运,不看位置** — 一份文件是设计基线还是过程产物,看它被消费几次
3. **漂移不是错,是数据** — 真实项目里命名漂移是数据,记录在 `aliases[]` 即可,不强迫改名
4. **证据存档真的只读** — `evidence_archive` 角色文件 chmod 444,改它之前先反思

## 5 类生命周期角色(完整工件映射见 schema)
...
`.xdd/xdd-schema.json:lifecycle_artifacts[]` 登记所有工件,5 角色,所有路径 + 别名。完整映射在 schema 里,本 skill 不重复抄。

```bash
# 列出所有设计基线工件
jq -r '.lifecycle_artifacts.artifacts[] | select(.role == "design_baseline") | .canonical_path' .xdd/xdd-schema.json
```
...
xdd-walker 干活时常问"我现在该写什么文件"。查 schema:
...
2. 占位符是否齐全(`{iter}` / `{slug}` / `{component}` 等)
...
| ID | 规则 | 触发器 | 行为 |
|----|------|--------|------|
| **R1** | 设计基线改动传播 | `stop-gate` 阶段扫描 | 检测 `.xdd/L*-*/` 设计基线文件 mtime 异常(> 24h 内多次修改),提示"考虑触发下游变更传播" |
| **R3** | 证据写阻断 | `post-write-stub-scan` 写入时 | 任何 Write/Edit 落到 `evidence_archive` 角色文件 → chmod 444 阻断 + 警告"先确认是否真要改证据" |
| **R5** | 漂移扫描 | `stop-gate` 末尾 | 漂移 ≥ 1 时 `exit 1` |
| **R6** | 路径 locality | `post-write-stub-scan` 写入时 | 任何 Write/Edit 落到 `.xdd/` 但不在 `canonical_path` 列表 → 警告"路径不在 schema 登记" |
| **R10** | 自动 `.archived` 锁 | iter 冻结时 | 自动给 `evidence_archive` 文件加 `.archived` 后缀,chmod 444 |

> 详细实现见 `scripts/gate-check-lifecycle.sh` + `hooks/xdd-gate-stop.sh` + `hooks/xdd-gate-stub-scan.sh` 的对应段。

## 产出

> **生命周期角色**:本 skill 本身 = `template_instance` 模板与实例 — `SKILL.md` + `references/*.md` + `scripts/gate-check-lifecycle.sh` 都是模板;xdd-walker 调用本 skill 跑出来的产物(角色判定报告 / 漂移清单 / 归档动作)归到 `.xdd/LIFECYCLE.md` 作为 `control_marker` 控制标记(供 session-start 引用)。

**调用本 skill 后落地的产物**:
- `.xdd/LIFECYCLE.md` — 项目内生命周期索引页(由 `xdd-init` 自动创建,本 skill 引用)
...
- **`xdd-init`**:本 skill 引用 `xdd-init` 创建的 `.xdd/LIFECYCLE.md`
- **`xdd-walker`**:Walker 装本 skill 查 schema,不在脑里记 5 类
- **`xdd-reviewer`**(待建):Reviewer chain 审计用本 skill 的角色分布作维度
- **`xdd-trace-init`**:反向追溯时按角色加权(`design_baseline` 权最高,`process_output` 权最低)
- **`xdd-reverse`**:逆向已有系统时,本 skill 帮判断哪些是"野生设计基线"(反推产物类别)

## 自检(本 skill 完成后)

- [ ] 5 角色都能在 schema 中查到定义
- [ ] 所有工件都在 schema 中登记
- [ ] `gate-check-lifecycle.sh` 跑出来 exit 0(无漂移)
- [ ] 任意给一个 .xdd/ 实物文件,`lifecycle_role_of` 能识别

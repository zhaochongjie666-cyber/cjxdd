...
| `session_compact` | manual/threshold/overflow compaction 后 | 发送 `COMPACTION_DONE`，恢复 Controller 的唯一 continuation。 |
| `input` | 输入解析、skill/template 展开前 | 丢弃暂停的 xdd steering/continuation；仅已交付的 follow-up continuation 清除 continuation lock。所有 handler 维持 `async`，以兼容 Pi 的异步 extension runner。 |

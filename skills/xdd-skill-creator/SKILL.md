...
├── DEPS.md (optional) - Runtime dependencies (npm, pip, system)
...
**DEPS.md**: When a skill's scripts require external packages (npm like `ws`, pip like `requests`, system tools like `ffmpeg`), create a `DEPS.md` next to `SKILL.md`. The `skill-deps` extension auto-detects these and offers to install missing dependencies. Format: section headers `## npm`, `## pip`, `## system`, `## commands` with list items. See `references/skill-deps.md` for details.
...
2. **Aggregate into benchmark** — run the aggregation script from the skill-creator directory:
   ...
   # 或从 skill 目录执行: python -m skills.skill-creator.scripts.aggregate_benchmark
   ```
...
1. Read the template from `eval-viewer/assets/eval_review.html`
...
python -m scripts.run_loop \
  ...
  --max-iterations 5 \
  --verbose
# 或从 skill 目录执行: python -m skills.skill-creator.scripts.run_loop
```
...
# 或从 skill 目录执行: python -m skills.skill-creator.scripts.package_skill
```

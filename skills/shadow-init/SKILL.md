...
脚本会：
1. 读 `.shadow/shadow-schema.json` 里的 `shadow_version` 字段
...
| `--schema PATH` | 自动找 | 指定 schema.json 路径（默认解软链找仓库根的 `.shadow/shadow-schema.json`） |
...
1. **模板从 schema.json 派生** — status.md 表的行数、字段都对应 `.shadow/shadow-schema.json` 的 `stages[]`，所以改 schema 不会导致 init 出来的 status.md 跟阶段表对不上。
...
| 现象 | 原因 | 修法 |
|------|------|------|
| `schema not found` | 没在 shadow 仓库里 / 软链解不开 | `pwd` 确认在仓库根；`ls .shadow/shadow-schema.json` 存在？ |

 ...
 * 命名收窄和收敛 NF 用到的 3 个 xdd 阶段名，不新增独立的状态机或 schema。
 *
...
/**
 * NF 用到的 xdd stage 名子集，按执行顺序排列。display name（framework/scenarios/
 * verify）只在用户面文案里出现，见 NF_DISPLAY_NAME；normal-flow-runtime.json /
 * StageContract / Controller 全程只认这里的 xdd 名，不引入新 type literal。
 */
export const NF_STAGE_NAMES = ["architecture", "spec", "verify"] as const;

...
# 1. 13 skill 存在
miss=0
for s in xdd-init xdd-understand xdd-spec xdd-architecture xdd-wire xdd-resilience xdd-plan xdd-execute xdd-verify xdd-reverse xdd-mermaid-check xdd-docker-helper xdd-skill-creator; do
  [ -f "skills/$s/SKILL.md" ] || { echo "  缺 skill: $s"; miss=1; }
done
n=$(ls -d skills/*/ 2>/dev/null | wc -l)
{ [ "$n" -eq 13 ] && [ "$miss" = 0 ]; }; check "1. 正好 13 个 skill (实际 $n)" "$?"

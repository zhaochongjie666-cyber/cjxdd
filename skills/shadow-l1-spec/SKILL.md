...
**生命周期角色**（`design_baseline` 设计基线）：RXX 规则是 L1.5 API 端点 / L2 验收 / L3 失败模式 / L5 Harness 计划 4 个下游的追溯键;改 RXX 一律触发全链传播。详见 `shadow-schema.json:lifecycle_artifacts` → `spec-bxx`。

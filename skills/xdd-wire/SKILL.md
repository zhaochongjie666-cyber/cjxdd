...
**我锚定的是「用户看到什么、怎么操作、每个状态长什么样」** —— 把业务规则变成可视的页面。每个页面的每个元素都要有存在的理由，每个状态（空/加载/错误/成功/确认/边界，6 态规范见 `references/operation-states.md`）都要画到，不留混淆。

| | |
|---|---|
| **上游** | `xdd-brainstorm`(intent.md) + `xdd-spec`(RXX 规则 + Feature 里的页面名/交互/角色) |
| **我产出** | `.xdd/design/wire/{page}/index.html` + 6 操作态 + `review.md` |
| **下游消费者** | `xdd-plan`（前端 task）、`xdd-verify`（页面渲染验收 + wire↔code 一致性） |
...
├── error.html           # 错误态
├── success.html         # 成功态
├── confirm.html         # 确认态（破坏性/不可逆操作前）
├── edge.html            # 边界态（输入超限/无权限/配额耗尽/数据溢出/离线）
└── review.md            # Step 3 攻击式 review
```

**6 态怎么画 → 查 `references/operation-states.md`（实操规范）**，核心：

- **6 态是交付门槛，不是加分项**——主页面只画理想态，真实用户大量时间在空/加载/错误/成功/确认/边界态。不画 = 用户碰到时一脸懵。

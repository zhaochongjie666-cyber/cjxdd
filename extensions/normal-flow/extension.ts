...
async function sendNfAdvanceNextStageSteering(
	...
	const stageSpecific = stage === "verify"
		? `已进入 verify 阶段。verify 的作用是防止偷工减料，不是写个报告说通过就通过。你要：1) 确保所有 Feature 都有真实实现（无桩/占位/注释）；2) 从不同用户角色（管理员/普通用户/审批者等）视角，通过 HTTP 端点或浏览器端到端验收每个 Feature 场景；3) 每个用户旅程都要截图（.png/.jpg），放在 .xdd/runs/normal_run/screenshots/ 下；4) 正向路径和兜底路径（拒绝/失败/无权限/边界）都要端到端验证；5) 有 Scenario 没做就是 P0（硬检查），必须回 scenarios 重做；P1/P2 由报告声明必须为 0；6) 所有用户旅途必须走通，走不通 = P0 = 回炉。立即调用 nf_observe、nf_desired_state、nf_difference 开始。`
